...
export function Header({title}: HeaderProps) {
  ...
  useEffect(() => {
    ...
    invoke<VolumeWarningPayload>('get_volume_warning')
      .then(response => setIsVolumeFull(response.is_full));
  ...
  return (
    <header className="flex justify-between h-fit w-full py-4 px-8 items-center">
        ...
        <div className={cn("bg-yellow-200 p-6 text-2xl font-bold", isVolumeFull && "hidden")}>
          音量が100%ではありません
        </div>
